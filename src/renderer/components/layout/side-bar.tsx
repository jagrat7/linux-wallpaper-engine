...
import { Download, ListVideo, Monitor, Settings } from 'lucide-react'
  ...
  return (
    <SidebarPrimitive collapsible="icon" className={cn('', className)}>
      <SidebarHeader className="gap-0 p-0">
        <div className="border-border/65 flex h-12 items-center justify-center border-b px-2 group-data-[collapsible=icon]:px-0">
          <div
            className={cn(
              'flex shrink-0 items-center justify-center rounded-md p-1',
              isLightTheme && 'invert-[0.1]',
            )}
          >
            <img src={logoImage} alt="Wallpaper Engine Logo" className="size-7 object-contain" />
          </div>
        </div>
      </SidebarHeader>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.to}>
                    <SidebarMenuButton
                      aria-keyshortcuts={getAriaKeyShortcut(item.shortcut)}
                      isActive={isActive}
                      tooltip={item.label}
