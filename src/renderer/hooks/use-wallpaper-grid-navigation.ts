...
export interface WallpaperGridItemProps {
  tabIndex: number
...
export function useWallpaperGridNavigation({
  ...
  const idToIndex = useMemo(() => new Map(itemIds.map((id, index) => [id, index])), [itemIds])
  ...
  const getItemProps = useCallback(
    (id: string, index: number): WallpaperGridItemProps => ({
      ...
      onKeyDown: (event) => {
        ...
        else if (event.key === 'ArrowDown' && index + columns < itemIds.length)
          nextIndex = index + columns
        else if (event.key === 'Home') nextIndex = event.metaKey || event.ctrlKey ? 0 : rowStart
        else if (event.key === 'End')
          nextIndex = event.metaKey || event.ctrlKey ? itemIds.length - 1 : rowEnd
        else return
