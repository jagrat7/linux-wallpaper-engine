...
import type {
  ...
  WorkshopItem as AppWorkshopItem,
...
import {
  ...
  parseWorkshopId,
  settleWorkshopPageResults,
  shuffleDiscoverSectionConfigs,
  toSafeNumber,
} from './workshop.utils'
...
class WorkshopService implements IWorkshopService {
  ...
  async query(options?: WorkshopQueryOptions): Promise<WorkshopQueryResult> {
      ...
      combinations.map((requiredTags) =>
        client.workshop.getAllItems(
          ...
          WALLPAPER_ENGINE_APP_ID,
          {
            ...
            rankedByTrendDays:
              queryType === UGC_QUERY_TYPE_RANKED_BY_TREND ? WORKSHOP_TREND_DAYS : undefined,
    ...
    const mergedRawItems = mergeWorkshopItemsBySource<
      NonNullable<WorkshopPaginatedResult['items'][number]>
    >(results.map((result: WorkshopPaginatedResult) => result.items))
    const items: AppWorkshopItem[] = mapWorkshopItems(mergedRawItems)
    const totalResults = results.reduce(
      (sum: number, result: WorkshopPaginatedResult) => sum + result.totalResults,
      0,
    )
    const cappedTotalResults = Math.min(totalResults, WORKSHOP_MAX_RESULTS)
    const resultsPerPage = Math.max(
      results.reduce(
        (sum: number, result: WorkshopPaginatedResult) => sum + result.returnedResults,
        0,
      ),
      1,
    )
    const returnedResults = items.length
    const anyCombinationHasMore = results.some(
      (result: WorkshopPaginatedResult) => page * result.returnedResults < result.totalResults,
    )
  ...
  async discover(options?: WorkshopDiscoverOptions): Promise<WorkshopDiscoverResult> {
    ...
    const fetchSection = async (
      ...
      const isPinned = PINNED_DISCOVER_SECTION_CONFIGS.some(
        (pinned) => pinned.id === sectionConfig.id,
      )
      const queryType = isPinned || !sortedQueryType ? sectionConfig.queryType : sortedQueryType
      const rankedByTrendDays =
      ...
      const hasNextPage = results.some(
        (result: WorkshopPaginatedResult) => page * result.returnedResults < result.totalResults,
      )
    ...
    const sections = await Promise.all(
      allConfigs.map((sectionConfig) =>
        fetchSection(sectionConfig, DISCOVER_PAGE, DISCOVER_SECTION_LIMIT),
      ),
    )
