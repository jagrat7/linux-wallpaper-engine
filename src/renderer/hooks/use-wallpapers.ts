...
export function filterAndSortWallpapers(
  ...
  if (
    ...
    result = result.filter((w) => {
      ...
      if (
        hasResolutionFilter &&
        !filterResolution.includes(
          !w.resolution.height || !w.resolution.width
            ? 'Unknown'
            : `${w.resolution.width}x${w.resolution.height}`,
        )
      )
        return false
...
export function useWallpapers() {
  const { data, isLoading, isFetching, error, refetch } = trpc.wallpaper.getWallpapers.useQuery()
