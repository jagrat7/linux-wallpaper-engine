...
function buildScreensTooltip(
  activeWallpapers: ScreenWallpaper[],
  activePlaylists: ScreenPlaylist[],
): string {
...
export function StatusBar({ className }: StatusBarProps) {
  ...
  const activeWallpaper =
    activeWallpapers.find((w) => w.screen === primaryDisplay?.name) ?? activeWallpapers[0]
  ...
  const otherActiveCount = activeWallpapers.filter(
    (w) => w.screen !== activeWallpaper?.screen,
  ).length
  ...
  return (
    <footer
      className={cn(
        'border-border bg-sidebar flex items-center justify-between border-t px-4',
        className,
      )}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          {activeWallpaper ? (
            ...
            <>
              <div className="bg-muted-foreground/50 size-2 rounded-full" />
              <span className="text-muted-foreground text-sm">No active wallpaper</span>
            </>
          )}
        </div>
      </div>
