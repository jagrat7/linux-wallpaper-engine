...
export function BackendNotInstalledDialog({ open, onOpenChange }: BackendNotInstalledDialogProps) {
  ...
  const handleOpenInstallGuide = async () => {
    await openExternalMutation.mutateAsync({
      url: 'https://github.com/jagrat7/linux-wallpaper-engine/blob/main/docs/README.md#2-install-linux-wallpaperengine',
    })
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border max-w-md">
        <DialogHeader className="gap-3 pr-6">
          <div className="border-destructive/20 bg-destructive/10 text-destructive flex size-10 items-center justify-center rounded-md border">
            <AlertTriangle className="size-5" />
          </div>
          <div className="grid gap-2">
            <DialogTitle>linux-wallpaperengine is missing</DialogTitle>
            <DialogDescription className="leading-6">
              {BACKEND_NOT_INSTALLED_ERROR_MESSAGE}. Install it, and/or copy the binary to your PATH
              (e.g., ~/bin), then try applying the wallpaper again.
            </DialogDescription>
          </div>
        </DialogHeader>
