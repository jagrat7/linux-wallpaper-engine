    ...
    <div className={className}>
      <div className="flex">
        <Button
          ...
          className={cn(
            'flex-1 gap-2 rounded-r-none',
            isActive && 'text-destructive hover:bg-destructive/10 hover:text-destructive',
          )}
          onClick={() =>
            isActive && onStop ? onStop(activeScreens) : onApply(targetScreen ?? undefined)
          }
          disabled={isApplying}
        >
          {isApplying ? (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-48">
                ...
                {displays.map((display) => {
                  ...
                  return (
                    <DropdownMenuItem
                      key={display.name}
                      onClick={() =>
                        isDisplayActive && onStop
                          ? onStop(display.name)
                          : handleApplyTo(display.name)
                      }
                    >
                      {isDisplayActive ? (
                        <Square className="size-4" />
                      ...
                      {display.primary && (
                        <span className="text-muted-foreground ml-auto text-xs">Primary</span>
                      )}
                    </DropdownMenuItem>
                  )
                })}
            ...
            <DropdownMenuItem
              onClick={() => (isActive && onStop ? onStop(activeScreens) : handleApplyTo())}
            >
