...
export function CompatibilityScanRow() {
  ...
  const statusCounts = report?.reduce(
    (acc, r) => {
    ...
    <div>
      <div className="space-y-3 px-4 py-3">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-muted-foreground truncate text-xs">{progress.current}</p>
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <AlertDialogTrigger asChild>
                <Button variant="outline" size="sm" disabled={scanMutation.isPending}>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will test each wallpaper by briefly running it in a small hidden window.
                    Wallpapers you&apos;ve already tagged will be skipped. This may take several
                    minutes depending on how many wallpapers you have.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
        ...
        {report && report.length > 0 && !isScanning && (
          <button
            className="bg-secondary/50 hover:bg-secondary flex w-full items-center justify-between rounded-lg px-3 py-2 text-xs transition-colors"
            onClick={() => setReportOpen(true)}
          >
            <div className="flex items-center gap-3">
              {statusCounts &&
                (['perfect', 'minor', 'major', 'broken'] as const).map((status) => {
