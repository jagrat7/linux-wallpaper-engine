...
export function WallpaperGrid() {
  ...
  const wallpapers: Wallpaper[] = useMemo(() => {
    const visibleWallpapers = transformedWallpapers.filter(
      (wallpaper) => !unsubscribedWorkshopIds.has(wallpaper.workshopId ?? wallpaper.id),
    )
  ...
  useEffect(() => {
    ...
    const { tags, resolutions } = rawWallpapers.reduce(
      (acc, item) => {
        if (item.tags) acc.tags.push(...item.tags)

        acc.resolutions.push(
          !item.resolution.height || !item.resolution.width
            ? 'Unknown'
  ...
  if (error) {
    return (
      <div className="flex h-full flex-col">
        ...
        <motion.div
          className="text-destructive flex flex-col items-center justify-center py-20"
