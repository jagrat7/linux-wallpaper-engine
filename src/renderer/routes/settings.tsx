...
const windowGeometrySchema = z.object({
  windowGeometry: z
    ...
    .trim()
    .refine(
      (value) => !value || /^[1-9]\d*x[1-9]\d*$/.test(value),
      'Use widthxheight, for example 800x600',
    ),
})
  ...
  return (
    <div className="max-h-[100vh] p-6">
      ...
      <div className="columns-1 gap-6 2xl:columns-2 [&>*]:mb-6 [&>*]:break-inside-avoid">
          ...
          <SettingRow
            label={
              <span className="inline-flex items-center gap-1">
                ...
                >
                  <ChevronDown
                    className={`size-3.5 transition-transform ${startupTrayOpen ? 'rotate-180' : ''}`}
                  />
                </Button>
              </span>
            }
          ...
          <SettingRow
            label={
              <span className="inline-flex items-center gap-1">
                ...
                >
                  <ChevronDown
                    className={`size-3.5 transition-transform ${windowGeometryOpen ? 'rotate-180' : ''}`}
                  />
                </Button>
              </span>
            }
        ...
        {/* Audio Section */}
        <SettingsSection icon={Volume2} title="Audio" description="Volume and audio processing">
        ...
        {/* Display Section */}
        <SettingsSection icon={Monitor} title="Display" description="Default display behavior">
          <SettingRow label="Maximum FPS">
            <SelectControl
              options={getFpsOptions(maxRefreshData?.maxRefreshRate ?? 60, settings.fps).map(
                (fps) => ({
                  label: `${fps} FPS`,
                  value: String(fps),
                }),
          ...
          <SettingRow label="Default scaling">
            <SelectControl
              ...
              onChange={(value) =>
                updateSetting('defaultScaling', value as AppSettings['defaultScaling'])
              }
              triggerClassName="w-28"
            />
          </SettingRow>
              ...
              {WALLPAPER_GRID_DENSITY_OPTIONS.map(({ label, value }) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'text-muted-foreground hover:text-foreground focus-visible:ring-ring inline-flex size-8 items-center justify-center rounded-md transition-colors focus-visible:ring-1 focus-visible:outline-none',
